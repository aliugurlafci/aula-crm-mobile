 ...
 * the config slice (mobileScreens travel on `/auth/me`). `useAuth()` reads this
 * slice and keeps its old shape, so screens are untouched.
 */
import { createAsyncThunk, createSlice, type PayloadAction, type ThunkDispatch, type AnyAction } from '@reduxjs/toolkit';
...
async function persistSession(dispatch: Dispatch, token: string, profile: Me): Promise<void> {
  ...
  dispatch(authSlice.actions.setSession({ me: profile, offline: false }));
}
...
export const bootstrapAuth = createAsyncThunk('auth/bootstrap', async (_arg, { dispatch }) => {
  ...
  if (cached) {
    ...
    dispatch(authSlice.actions.setSession({ me: cached, offline: true })); // show cached UI at once
  }
  try {
    const fresh = await auth.me();
    await kvSet('me', fresh);
    applyProfile(dispatch, fresh);
    dispatch(authSlice.actions.setSession({ me: fresh, offline: false }));
  } catch (err) {
    if (err instanceof ApiRequestError && err.status === 401) {
      ...
      dispatch(authSlice.actions.setUnauthenticated()); // no cache + unreachable → logged out
    }
...
export const loginWithCredentials = createAsyncThunk<LoginResult, { email: string; password: string; code?: string }>(
  ...
  async ({ email, password, code }, { dispatch }) => {
    try {
      ...
      return finishLogin(dispatch as Dispatch, token ?? null);
    } catch (err) {
